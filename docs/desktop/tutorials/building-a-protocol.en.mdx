...
summary: ' This tutorial will guide you through the process of creating a
  ...
  here](/assets/protocols/Sample-Protocol-v2.netcanvas). '
prerequisites: >2-
   This tutorial assumes you are familiar with the general concepts of the Network Canvas software, have read the [project overview](/en/desktop/getting-started/project-overview), and have read the tutorial on [using Interviewer](/en/desktop/tutorials/using-interviewer).
...
<StandAloneImage
  src="/assets/img/architect-guide/start-screen.png"
  content="The Architect start screen"
/>
...
<KeyConcept title='Key Concept: Node and Edge "Types"'>
  Part of the ontological flexibility of the Network Canvas suite is the ability
  to model unlimited numbers of node and edge types. These types are top level
  categories that differentiate nodes and edges from one-another. Common node
  types might include "person", "place", or "organization", but these categories
  are entirely up to you to decide upon.
</KeyConcept>
...
<KeyConcept title="Key Concept: Prompts">
  Prompts can contain formatting such as bold or italicized text to create
  emphasis and rhythm. To create this formatting, use markdown syntax.
</KeyConcept>
...
<KeyConcept title="Key Concept: Preview Mode">
...
<TipBox>
  At any point during this tutorial, free free to click the &quot;Save
  Changes&quot; button in the bottom bar when on the timeline view, or select
...
<KeyConcept title="Key Concept: Input controls">
...
<TipBox>
  Refer to our Interface Documentation for more in depth instruction, including
  useful Do's and Don'ts of working with [roster
  data](/en/desktop/interface-documentation/name-generator-roster).
</TipBox>
...
To demonstrate the range of input control options, we implemented far more fields than are likely to be best practice for a form within an interview. You can [learn more about input controls here](/en/desktop/key-concepts/input-controls)
...
<TipBox danger>
  You should think carefully about how the choices you make regarding the
  background will impact the way your participants position their nodes.
</TipBox>
...
<KeyConcept title="Key Concept: 'Bins' and 'Buckets'">
...
In the next tutorial, [Server and Interviewer Workflows](/en/desktop/tutorials/using-server), we will show you how to use Server to load a protocol file onto multiple devices and then receive the data from those devices.
