...
Because it is not a true 'server' application, Server has no special system requirements beyond the rest of the applications in the Network Canvas suite. That means you should follow the requirements listed in the [installation guide](../installation-guide#system-requirements).
...
For more information on these topics, see our articles on [IRB and security best practices](../reference/irb-best-practices) and [configuring your devices prior to starting data collection](../how-to/configuring-devices).
...
<ImageFullWith
  src="/assets/img/server-guide/server-welcome.png"
  name="The setup screen"
/>
...
<StandAloneImage
  src="/assets/uploads/screen-shot-2021-04-29-at-4.55.48-pm.png"
  content="Quitting the app from the system tray"
/>
...
<StandAloneImage
  src="/assets/img/server-guide/imported-protocol-server.png"
  content="The overview tab of a newly created workspace"
/>
...
<TipBox>
  For testing purposes, and to follow along with this tutorial, you may wish to
  run Interviewer and Server on the same computer, which will avoid the need to
...
<TipBox>
  Click the networking status indicator in the top right of Server to see
  information about the status of the automatic discovery feature, as well as
  other useful networking information.
</TipBox>
...
<StandAloneImage
  src="/assets/img/key-concepts/pairing/nc-manual-pairing.png"
  content="The manual connection details form"
/>
...
<StandAloneImage
  src="/assets/img/key-concepts/pairing/server-manual-details.png"
  content="Network information panel in Server"
/>

<TipBox>
  Please note that the connection information listed here may not reflect the
  publicly routable address for Server if you have configured NAT or port
  forwarding through a gateway.
</TipBox>
...
<StandAloneImage
  src="/assets/img/key-concepts/pairing/server-acknowledge.png"
  content="The pairing request is shown as a notification in Server"
/>
...
<StandAloneImage
  src="/assets/img/server-guide/key-server.png"
  content="The pairing code, shown in Server"
/>
...
<TipBox>
  You can copy and paste the pairing code from within Server. This may make
  entering it less error-prone.
</TipBox>
...
<StandAloneImage
  src="/assets/img/server-guide/import-protocol-interviewer.png"
  content="The updated Server section, and new protocol import option in Interviewer"
/>
...
As per the [offline workflow](./offline-data-management-workflow) it is also possible to import interview data into Server manually.
...
<StandAloneImage
  src="/assets/img/server-guide/export-data.png"
  content="The data export tab"
/>
...
- **GraphML**. An XML based open standard for representing graph data, that is compatible with many social network analysis programs including UCINET, Gephi, NodeXL, Pajek, Visone, and ORA. Server outputs a [slightly modified](../reference/network-canvas-graphml) version of the GraphML format, which also contains ego data and Network Canvas session metadata. The filename will be in the format `caseID_sessionUUID.graphml`.
...
<TipBox danger>
  It is **very** easy to create unmanageably large files by combining the "merge
  sessions by protocol" and "adjacency matrix" export options. A study
  containing 2000 cases will produce a single 13gb CSV file when using these
  options together! Ensure that you require both options before enabling them.
</TipBox>
...
- Learn about how to analyze the data you have exported from Server in the tutorial on [working with Network Canvas data in R](./working-with-data).
- Consult the tutorial on the [offline workflow](./offline-data-management-workflow) for further information about alternative approaches to managing study data, both with and without using Server.
- Read the advanced tutorial on [resolving entities across interviews](./resolving-entities) if you are interested in creating pseudo whole-networks from a collection of ego networks.
